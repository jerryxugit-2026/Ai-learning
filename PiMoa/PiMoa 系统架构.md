...
### 5.5 推理（reasoning / thinking）为何保持关闭 —— 实测定标（2026-07-25）

**结论：PiMoa 全链保持推理关闭。理由是延迟稳定性，不是质量。**

**实测**：3 个任务 × 4 种配置 = 12 次调用。任务①找并发缺陷（代码）／②安全审计（有标准答案：`assembly.go:688`）／③架构权衡（开放式）。

| 任务 | 配置 | 耗时 | 推理 tok | 质量要素 |
|---|---|---|---|---|
| ① | DeepSeek 关（当前） | 36.8s | 0 | 3/4 |
| ① | DeepSeek 开+high | 23.0s | 91 | 3/4 |
| ① | gemini 默认（当前） | 80.8s | 12,736 | **3/4（唯一指出根因）** |
| ① | gemini effort=high | 13.8s | 1,943 | **2/4（反而更差）** |
| ② | DeepSeek 关／开 | 9.9s／14.5s | 0／716 | 3/4／3/4 |
| ② | gemini 默认／high | 47.1s／6.7s | 1,315／0 | 4/4／4/4 |
| ③ | DeepSeek 关／开 | 30.9s／**109.0s** | 0／5,549 | 4/4／4/4 |
| ③ | gemini 默认／high | 17.8s／6.5s | 2,350／0 | 4/4／4/4 |

**读数**：① **开与关的质量要素命中数完全相同**，12 次无一例外（唯一差异是任务① gemini 开 high 后掉到 2/4）；② 延迟代价**极不稳定**——DeepSeek 在任务③ 从 30.9s 涨到 **109.0s（3.5 倍）**，烧 5,549 推理 token 换同样的 4/4。**proposer 并行取 max ⇒ 一个慢的拖垮整轮**，故稳定性优先。

**原理**：推理的价值在于**填补信息空白**（多步推导、试错回溯）。而 §4.9 侦查前置已把带行号的完整材料喂满——**答案在材料里，需要的是"看仔细"而非"想很久"**，思考链无用武之地；反而会 ① 与正文竞争输出预算（实测 DeepSeek flash 推理吃光 → 空正文）② 长链发散丢掉最直接的观察（任务① 即是）。**侦查前置做得越好，推理的边际价值越低。**

**两个易踩的坑**：
1. **`reasoning:false` 会让 `thinkingLevel` 完全失效**——pi 的 `models.ts:664` `if (!model.reasoning) return ["off"]` 直接锁死可选档。故 proposer（gemini/MiniMax，标 false）跑的是**模型自身默认档**，我们无控制权；`session.ts` 的 `thinkingLevel:"off"` 真正作用于**聚合器** deepseek（标 true）——pi 据此发 `thinking:{type:"disabled"}`。
2. **显式传 `reasoning_effort:"high"` 未必更"高"**：各服务商映射不一致，实测 gemini 传 high 后 `reasoning_token` 反而变 **0**（被映射成关闭）。**不传比传安全。**

> ⚠️ **适用边界**：本结论只对 **code review 类任务**（答案在给定材料内、以事实核对与模式识别为主）成立。**论文审核**那类需要重建论证链条、判断方法论适配性、做反事实推断的任务，答案**部分在材料之外**，推理的边际收益可能相反——**换场景必须重新实测，勿直接套用**（PaperGo/Hermes 侧尤其如此，其模型组合与参数均不同）。
